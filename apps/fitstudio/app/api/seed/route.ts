...
/**
 * One-shot seed endpoint. Protected by the `SEED_TOKEN` env var so it can only
 * be triggered by someone who knows the value. Idempotent — re-running it
 * upserts by stable keys, so hitting the URL multiple times is safe.
 *
 * Usage: GET /api/seed?token=<SEED_TOKEN>
 */
export async function GET(request: Request): Promise<Response> {
  const token = process.env.SEED_TOKEN;
  if (!token) {
    return NextResponse.json(
      { error: "SEED_TOKEN env var is not set" },
      { status: 503 },
    );
  }
